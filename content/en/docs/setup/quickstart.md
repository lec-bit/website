...
### Mode change
We provide two modes, ads and workload, for users to choose from. The default mode is currently workload. If we wishes to use the ads mode,  only need to modify a single startup parameter.

```shell
# Install from Helm
vim deploy/helm/valuse.yaml
...
